...
public abstract class NativeOCKAdapter implements NativeInterface {
    ...
    public void KEM_encapsulate(long PKeyId, byte[] wrappedKey, byte[] randomKey)
            throws OCKException {
        NativeOCKImplementation.KEM_encapsulate(ockContext.getId(), PKeyId, wrappedKey, randomKey);
    }

    @Override
    public byte[] KEM_decapsulate(long PKeyId, byte[] wrappedKey)
            throws OCKException {
        return NativeOCKImplementation.KEM_decapsulate(ockContext.getId(), PKeyId, wrappedKey);
    }

    @Override
    public byte[] PQC_SIGNATURE_sign(long PKeyId, byte[] data)
            throws OCKException {
        return NativeOCKImplementation.PQC_SIGNATURE_sign(ockContext.getId(), PKeyId, data);
    }

    @Override
    public boolean PQC_SIGNATURE_verify(long PKeyId, byte[] sigBytes, byte[] data)
            throws OCKException {
        return NativeOCKImplementation.PQC_SIGNATURE_verify(ockContext.getId(), PKeyId, sigBytes, data);
    }
}
