...
/* Note: when making ICC_D2i calls do not pass the address of a pointer
 allocated by GetPrimitiveArrayCritical.  Instead make a copy and pass the
 address of the copied pointer For Example,  instead of  passing
 parameterBytesNative, first create a copy:  pParameterBytes =
 parameterBytesNative and pass &pParameterBytes. d2i calls increment the
 pointer.*/
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1generate__JI(
...
#pragma convert(pop)
    ...
    if (nid == 0) {
        ...
        throwOCKException(env, 0,
                          "ICC_EC_Generate_key(ICC_OBJ_txt2nid) failed");
    } else {
#ifdef DEBUG_EC_DETAIL
        if (debug) {
...
#ifdef DEBUG_EC_DETAIL
            if (debug) {
                gslogMessage("DETAIL_EC rc from ICC_EC_generate_key=%d",
                             (int)rc);
            }
#endif
            if (rc != 1) {
...
#endif
                if (ockECGroup != NULL) {
                    ICC_EC_GROUP_set_asn1_flag(ockCtx,
                                               (ICC_EC_GROUP *)ockECGroup, 1);
                }
                ...
                //       }
                //       ecKeyId = 0;
                //       throwOCKException(env, 0, "ICC_EC_Generate_key
                //       (ICC_EC_KEY_check_key) failed");
                // }
                // else {
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1generate__JLjava_lang_String_2(
    ...
    if (nativeSoid == NULL) {
...
#endif
        throwOCKException(env, 0,
                          "ICC_EC_Generate_key (GetStringUTFChars) failed");
    } else {
#ifdef DEBUG_EC_DETAIL
        if (debug) {
        ...
        if (nid <= 0) {
            ...
            if (ockECKey == NULL) {
                ...
                if (debug) {
                    gslogMessage(
                        "DETAIL_EC FAILURE ICC_EC_KEY_new_by_curve_name");
                }
#endif
                ...
                if (debug) {
                    gslogMessage(
                        "DETAIL_EC rc from ICC_EC_KEY_generate_key  %d", rc);
                }
#endif
                if (rc != 1) {
                    (*env)->ReleaseStringUTFChars(env, soid, nativeSoid);
                    ...
                    if (debug) {
                        gslogMessage(
                            "DETAIL_EC FAILURE ICC_EC_KEY_generate_key %d", rc);
                    }
#endif
...
#ifdef DEBUG_EC_DETAIL
                        if (debug) {
                            gslogMessage("DETAIL_EC ockECGroup %lx",
                                         (long)ockECGroup);
                        }
#endif
                        ICC_EC_GROUP_set_asn1_flag(
                            ockCtx, (ICC_EC_GROUP *)ockECGroup, 1);
                    } else {
#ifdef DEBUG_EC_DETAIL
                        if (debug) {
...
int getPublicKey(ICC_CTX *ockCtx, JNIEnv *env, ICC_EVP_PKEY *key,
...
// helper fuction for XECKEY_generate
int getDERPublicKey(ICC_CTX *ockCtx, JNIEnv *env, ICC_EVP_PKEY *key,
                    unsigned char *buffer) {
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_XECKEY_1generate(
...
#pragma convert("ISO8859-1")
#endif
                rc = ICC_EVP_PKEY_CTX_ctrl_str(ockCtx, pctx, "dh_param",
                                               secondaryOption);
#ifdef __MVS__
#pragma convert(pop)
#endif
            ...
            if (rc == 1) {
                ...
                if (rc == 1) {
                    if (mainOption == NID_ffdh) {
                        rc = getDERPublicKey(ockCtx, env, key,
                                             (unsigned char *)bufferPtr);
                    } else {
                        rc = getPublicKey(ockCtx, env, key,
                                          (unsigned char *)bufferPtr);
                    }
                    if (rc > 0) {
                        if (debug) {
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1generateParameters__JI(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jint numBits) {
...
#pragma convert("ISO8859-1")
#endif
    static char *curveNames[] = {"prime192v1", "prime224v1", "prime256v1",
                                 "prime384v1", "prime581v1"};
#ifdef __MVS__
#pragma convert(pop)
#endif
    int curveidx = 2;
...
#ifdef DEBUG_EC_DETAIL
    if (debug) {
        gslogMessage("DETAIL_EC curveidx=%d curveName=%s", curveidx,
                     curveNames[curveidx]);
    }
#endif
    ...
    if (nid == 0) {
        ...
        if (ockECKey == NULL) {
            ...
            if (debug) {
                ...
                gslogMessage("DETAIL_EC size from ICC_i2d_ECParameters=%d",
                             size);
            }
#endif
                ...
                if (parmBytes == NULL) {
                    ...
                    if (parmBytesNative == NULL) {
                        ...
                        if (size <= 0) {
                            ...
                            if (debug) {
                                gslogMessage(
                                    "DETAIL_EC FAILURE ICC_i2d_ECParameters %d",
                                    size);
                            }
#endif
                            throwOCKException(env, 0,
                                              "ICC_i2d_ECParameters failed");
                        } else {
                            retParmBytes = parmBytes;
#ifdef DEBUG_EC_DATA
                            if (debug) {
                                gslogMessage(
                                    "DATA_EC size from ICC_i2d_ECParameters(2) "
                                    "=%d",
                                    size);
                                gslogMessagePrefix("DATA_EC parmBytes : ");
                                gslogMessageHex((char *)parmBytes, 0, size, 0,
                                                0, NULL);
                            }
#endif
                        }
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1generateParameters__JLjava_lang_String_2(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jstring soid) {
    static const char *functionName =
    ...
    if (soid == NULL) {
        throwOCKException(env, 0,
                          "Generating EC parameters failed. The specified "
                    ...
                    if (parmBytes == NULL) {
                        ...
                        if (parmBytesNative == NULL) {
                            ...
                            unsigned char *pBytes =
                                (unsigned char *)parmBytesNative;

                            size =
                                ICC_i2d_ECParameters(ockCtx, ockECKey, &pBytes);
#ifdef EC_DEBUG
                            if (debug) {
                                gslogMessage(
                                    "DETAIL_EC size from "
                                    "ICC_i2d_ECParameters(2) =%d",
                                    size);
                            }
#endif
                            if (size <= 0) {
                                ...
                                if (debug) {
                                    gslogMessagePrefix("DATA_EC parmBytes : ");
                                    gslogMessageHex((char *)parmBytes, 0, size,
                                                    0, 0, NULL);
                                    gslogFunctionExit(functionName);
                                }
#endif
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1generate__J_3B(
    ...
    if (parameterBytes == NULL) {
        throwOCKException(env, 0,
                          "The specified input parameters are incorrect.");
        if (debug) {
            gslogFunctionExit(functionName);
        }
        return ecKeyId;
    }
    parameterBytesNative = (unsigned char *)((*env)->GetPrimitiveArrayCritical(
        env, parameterBytes, &isCopy));
    if (parameterBytesNative == NULL) {
        ...
        if (ockECKey == NULL) {
            ...
            // ockECGroup = ICC_d2i_ECPKParameters (ockCtx, NULL, &pBytes,
            // size);
...
#ifdef DEBUG_EC_DETAIL
                if (debug) {
                    gslogMessage("DETAIL_EC rc from ICC_EC_KEY_generate_key=%d",
                                 rc);
                }
#endif
                if (rc != 1) {
                    //(*env)->ReleasePrimitiveArrayCritical(env, parameterBytes,
                    // parameterBytesNative, 0);
                    ockCheckStatus(ockCtx);
#ifdef DEBUG_EC_DETAIL
                    if (debug) {
                        gslogMessage(
                            "DETAIL_EC FAILURE ICC)+EC_KEY_generate_key %d",
                            rc);
                    }
#endif
                    throwOCKException(
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1createPrivateKey(
    JNIEnv *env, jclass thisObj, jlong ockContextId,
    ...
    if (privateKeyBytes == NULL) {
        throwOCKException(env, 0,
                          "Creating EC Private Key failed. The specified input "
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_XECKEY_1createPrivateKey(
    ...
    if ((privateKeyBytes == NULL) || ((unsigned char *)bufferPtr == NULL)) {
        throwOCKException(env, 0,
                          "Creating XEC Private Key failed. The specified "
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1createPublicKey(
    ...
    if ((publicKeyBytes == NULL) || (parameterBytes == NULL)) {
        throwOCKException(env, 0,
                          "Creating EC Public Key failed. The specified input "
                          "parameters are incorrect.");
        if (debug) {
            gslogFunctionExit(functionName);
        }
    ...
    if (keyBytesNative == NULL) {
        ...
        if (parameterBytesNative == NULL) {
            ...
            ockECKey = ICC_d2i_ECParameters(
                ockCtx, &ockECKey, (const unsigned char **)&pParamBytes,
                (long)paramsize);
#ifdef DEBUG_EC_DETAIL
            if (debug) {
                gslogMessage("DETAIL_EC ockECKey from ICC_d2i_ECParameters=%lx",
                             (long)ockECKey);
            }
#endif
            if (ockECKey == NULL) {
                ...
                ockECKey = ICC_o2i_ECPublicKey(ockCtx, &ockECKey, &pKeyBytes,
                                               (long)size);
#ifdef DEBUG_EC_DETAIL
                if (debug) {
                    gslogMessage(
                        "DETAIL_EC ockECKey from ICC_o2i_ECPublicKey=%lx",
                        (long)ockECKey);
                }
#endif
                if (ockECKey == NULL) {
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_XECKEY_1createPublicKey(
    JNIEnv *env, jclass thisObj, jlong ockContextId,
    ...
    if (publicKeyBytes == NULL) {
        throwOCKException(env, 0,
                          "Creating XEC Public Key failed. The specified input "
                          "parameters are incorrect.");
        if (debug) {
            gslogFunctionExit(functionName);
        }
...
#ifdef DEBUG_EC_DETAIL
            if (debug) {
                gslogMessage("DETAIL_XEC FAILURE ICC_d2i_XDHPublicKey");
            }
#endif
...
#ifdef DEBUG_EC_DETAIL
            if (debug) {
                gslogMessage("DETAIL_XEC xecKeyId=%lx", (long)xecKeyId);
            }
#endif
        }
    }
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1getParameters(
    ...
    if (ockECKey == NULL) {
        throwOCKException(env, 0,
                          "EC Key getting parameters failed. The specified "
                          "input parameters are incorrect.");
        if (debug) {
            gslogFunctionExit(functionName);
        }
        ...
        if (parmBytes == NULL) {
            ...
            if (parmBytesNative == NULL) {
                ...
                if (size <= 0) {
                    ...
                    if (debug) {
                        gslogMessage(
                            "DETAIL_EC FAILURE ICC_i2d_ECParmeters size=%d",
                            size);
                    }
#endif
                    throwOCKException(env, 0, "ICC_i2d_ECParameters failed");
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1getPrivateKeyBytes(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ecKeyId) {
    static const char *functionName =
    ...
    if (ockECKey == NULL) {
        throwOCKException(env, 0,
                          "The specified EC Key identifier is incorrect.");
        if (debug) {
            gslogFunctionExit(functionName);
        }
        return retKeyBytes;
        ...
        if (keyBytes == NULL) {
            ...
            if (keyBytesNative == NULL) {
                ...
                if (size <= 0) {
                    ...
                    if (debug) {
                        gslogMessage(
                            "DETAIL_EC FAILURE ICC_i2d_ECPrivateKey size %d",
                            size);
                    }
#endif
                    throwOCKException(env, 0, "ICC_i2d_ECPrivateKey failed");
                    ...
                    if (debug) {
                        gslogMessage(
                            "DATA_EC size from ICC_i2d_ECPrivateKey %d",
                            (int)size);
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_XECKEY_1getPrivateKeyBytes(
    ...
    if (ockEVPKey == NULL) {
        throwOCKException(env, 0,
                          "XEC Key getting Private Key bytes failed. The "
                          "specified input parameters are incorrect.");
        if (debug) {
            gslogFunctionExit(functionName);
        }
        return retKeyBytes;
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1getPublicKeyBytes(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ecKeyId) {
...
#ifdef DEBUG_EC_DETAIL
    if (debug) {
        gslogMessage("DETAIL_EC size from ICC_i20_ECPublicKey(NULL)=%d",
                     (int)size);
    }
#endif
    if (size <= 0) {
...
#ifdef DEBUG_EC_DETAIL
                if (debug) {
                    gslogMessage("DETAIL_EC size from ICC_i2o_ECPublicKey=%d",
                                 (int)size);
                }
#endif
                if (size <= 0) {
...
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_XECKEY_1getPublicKeyBytes(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong xecKeyId) {
    ICC_CTX       *ockCtx         = (ICC_CTX *)((intptr_t)ockContextId);
 ...
 */
JNIEXPORT jlong JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1createPKey(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ecKeyId) {
...
#ifdef DEBUG_EC_DETAIL
            if (debug) {
                gslogMessage("DETAIL_EC FAILURE ICC_EVP_PKEY_set1_EC_KEY rc=%d",
                             rc);
            }
#endif
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1delete(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ecKeyId) {
 ...
 */
JNIEXPORT void JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_XECKEY_1delete(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong xecKeyId) {
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1computeECDHSecret(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong pubEcKeyId,
    jlong privEcKeyId) {
    static const char *functionName =
        "NativeInterface_ECKEY_1computeECDHSecret";

    ICC_CTX          *ockCtx      = (ICC_CTX *)((intptr_t)ockContextId);
    ICC_EC_KEY       *ockPubEcKey = (ICC_EC_KEY *)((intptr_t)pubEcKeyId);
    const ICC_EC_KEY *ockPrivEcKey =
        (const ICC_EC_KEY *)((intptr_t)privEcKeyId);
...
#endif
    if (ockEcPoint == NULL) {
        /* fprintf (stderr, "ockEcPoint is null; ICC_EC_KEY_get0_public_key
         * failed\n");*/
        ockCheckStatus(ockCtx);
#ifdef DEBUG_EC_DETAIL
        if (debug) {
        ...
        ockEcGroup =
            ICC_EC_KEY_get0_group(ockCtx, (const ICC_EC_KEY *)ockPrivEcKey);
        if (ockEcGroup == NULL) {
            size = 32;
#ifdef DEBUG_EC_DETAIL
            if (debug) {
        ...
        if (secretBytes == NULL) {
            ...
            if (secretBytesNative == NULL) {
...
#ifdef DEBUG_EC_DETAIL
                if (debug) {
                    gslogMessage("DETAIL_EC ockEcPoint=%ld ockPrivEcKey %x",
                                 (long)ockEcPoint, ockPrivEcKey);
                }
#endif

                lena = ICC_ECDH_compute_key(ockCtx, pBytes, size, ockEcPoint,
                                            (ICC_EC_KEY *)ockPrivEcKey, NULL);
#ifdef DEBUG_EC_DETAIL
                if (debug) {
                ...
                if (lena == -(ICC_ERROR_FLAG)) {
                    ...
                    if (debug) {
                        gslogMessage(
                            "DETAIL_EC FAILURE lena ICC_ECDH_compute_key %d",
                            lena);
                    }
#endif
                    throwOCKException(env, 0,
                                      "ICC_EC_KEY_get0_public_key failed");
                } else {
#ifdef DEBUG_EC_DATA
                    if (debug) {
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_XECKEY_1computeECDHSecret(
    ...
    gen_ctx = ICC_EVP_PKEY_CTX_new(ockCtx, (ICC_EVP_PKEY *)ockPrivXecKey,
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1signDatawithECDSA(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jbyteArray digestBytes,
    jint digestBytesLen, jlong privEcKeyId) {
    static const char *functionName =
        "NativeInterface_ECKEY_1signDatawithECDSA";
...
#ifdef DEBUG_EC_DETAIL
    if (debug) {
        gslogMessage("DETAIL_EC privEcKeyId=%lx, digestBytesLen=%d",
                     privEcKeyId, (int)digestBytesLen);
    }
#endif
    ...
    if (digestBytesNative == NULL) {
        ...
        if (size != digestBytesLen) {
            ...
            if (debug) {
                gslogMessage(
                    "DETAIL_EC FAILURE Internal inconsistency while validating "
                    "the digest length parameter %d %d",
                    (int)size, (int)digestBytesLen);
            }
#endif
            throwOCKException(env, 0,
                              "Internal inconsistency while validating the "
                              "digest length parameter");
        } else {
            ...
            if (sigBytes == NULL) {
                ...
                if (sigBytesNative == NULL) {
                    ...
                    pBytesSig = (unsigned char *)sigBytesNative;
                    rc        = ICC_ECDSA_sign(
                        ockCtx, type, pBytesDigest, size, pBytesSig,
                        (unsigned int *)&retSigBytesLen, ockPrivEcKey);
#ifdef DEBUG_EC_DATA
                    if (debug) {
                        gslogMessage("DATA_EC ICC_ECDSA_sign rc=%d", rc);
                        gslogMessagePrefix("DATA_EC sigBytes length =%d",
                                           retSigBytesLen);
                        gslogMessageHex((char *)pBytesSig, 0,
                                        (int)retSigBytesLen, 0, 0, NULL);
                    }
#endif
                    if (rc != 1) {
                        ockCheckStatus(ockCtx);
                        ...
                        if (debug) {
                            gslogMessage(
                                "DETAIL_EC FAILURE ICC_ECDSA_sign rc %d", rc);
                        }
#endif
                        ...
                        */
                        ...
                        if (digestBytesNative != NULL) {
                            (*env)->ReleasePrimitiveArrayCritical(
                                env, digestBytes, digestBytesNative, 0);
                        }
                        if (sigBytesNative != NULL) {
                            (*env)->ReleasePrimitiveArrayCritical(
                                env, sigBytes, sigBytesNative, 0);
                        }
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_ECKEY_1verifyDatawithECDSA(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jbyteArray digestBytes,
...
{
    static const char *functionName =
        "NativeInterface_ECKEY_1verifyDatawithECDSA";
...
#ifdef DEBUG_EC_DETAIL
        if (debug) {
            gslogMessage("DETAIL_EC size=%d digestBytesLen %d", size,
                         (int)digestBytesLen);
        }
#endif
        if (size != digestBytesLen) {
            ...
            if (sigBytesNative == NULL) {
                ...
                verified =
                    ICC_ECDSA_verify(ockCtx, type, pBytesDigest, size,
                                     pBytesSig, sigBytesLen, ockPubEcKey);
#ifdef DEBUG_EC_DETAIL
                if (debug) {
