...
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1create(
    ...
    static const char *functionName = "NativeOCKImplementation.CIPHER_create";
...
#endif
    if (cipherNameChars != NULL && strstr(cipherNameChars, "AES") != NULL &&
        strstr(cipherNameChars, "CBC") != NULL) {
...
#pragma convert(pop)
    ...
    if ((0 == ockCipher->copy_context) && (NULL == ockCipher->cached_context) &&
        (NULL != ockCipher->cipherCtx)) {
        // cipherCtx succeeded but could not allocate cached_context, then free
        // previously allocated cipherCtx
...
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1init(
    ...
    static const char *functionName = "NativeOCKImplementation.CIPHER_init";
    ...
    if ((ockCipher == NULL) || (key == NULL)) {
        throwOCKException(env, 0,
                          "The specified Cipher arguments are incorrect.");
        if (debug) {
            gslogFunctionExit(functionName);
        }
        return;
    ...
    if (NULL == keyNative) {
        ...
        if (rc != ICC_OSSL_SUCCESS) {
            ...
            if (rc == ICC_OSSL_SUCCESS) {
                rc = ICC_EVP_CIPHER_CTX_set_padding(
                    ockCtx, ockCipher->cipherCtx, ockPadType);
                if (rc != ICC_OSSL_SUCCESS) {
                    ockCheckStatus(ockCtx);
                ...
                if (0 == ockCipher->copy_context) {
                    if (NULL == ockCipher->cached_context) {
                        ockCipher->cached_context =
                            ICC_EVP_CIPHER_CTX_new(ockCtx);
                    }
                    if (NULL == ockCipher->cached_context) {
                        ockCheckStatus(ockCtx);
                        throwOCKException(env, 0,
                                          "ICC_EVP_CIPHER_CTX_new failed for "
                                          "CIPHER_init cached context");
                    }
                    rc = ICC_EVP_CIPHER_CTX_copy(ockCtx,
                                                 ockCipher->cached_context,
                                                 ockCipher->cipherCtx);
                    if (rc != ICC_OSSL_SUCCESS) {
                        ockCheckStatus(ockCtx);
...
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1setPadding(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId,
    jint paddingId) {
    static const char *functionName = "NativeOCKImplementation.CIPHER_setPadding";
    ...
    if (ockCipher == NULL) {
        ...
        throwOCKException(env, 0,
                          "The specified Cipher identifier is incorrect.");
        return;
    }
    switch (paddingId) {
        case PADDING_NONE:
 ...
 */
JNIEXPORT void JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1clean(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId) {
    static const char *functionName = "NativeOCKImplementation.CIPHER_clean";
...
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1getBlockSize(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId) {
    static const char *functionName = "NativeOCKImplementation.CIPHER_getBlockSize";
...
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1getKeyLength(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId) {
    static const char *functionName = "NativeOCKImplementation.CIPHER_getKeyLength";
 ...
 */
JNIEXPORT jint JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1getIVLength(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId) {
    static const char *functionName = "NativeOCKImplementation.CIPHER_getIVLength";
 ...
 */
JNIEXPORT jint JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1getOID(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId) {
    static const char *functionName = "NativeOCKImplementation.CIPHER_getOID";
...
JNIEXPORT jint JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_z_1kmc_1native(
    ...
    len = (jint)CIPHER_zKMC_internal(
        inputPointer + inputOffset, outputPointer + outputOffset,
        (int)inputLength, (long)paramPointer, (int)mode);
...
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1encryptUpdate(
    ...
    static const char *functionName = "NativeOCKImplementation.CIPHER_encryptUpdate";
    ...
    if (NULL == ciphertextNative || NULL == plaintextNative) {
        ...
        returnResult = CIPHER_encryptUpdate_internal(
            ockCtx, ockCipher, plaintextNative + (int)plaintextOffset,
            (int)plaintextLen, ciphertextNative + (int)ciphertextOffset,
            needsReinit);
...
JNIEXPORT int CIPHER_encryptFinal_internal(
    ...
    rc = ICC_EVP_EncryptFinal(ockCtx, ockCipher->cipherCtx,
                              ciphertext + updateOutlen, &finalOutlen);
    if (ICC_OSSL_SUCCESS != rc) {
...
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1encryptFinal(
    ...
    jbyteArray plaintext, jint plaintextOffset, jint plaintextLen,
    jbyteArray ciphertext, jint ciphertextOffset, jboolean needsReinit) {
    static const char *functionName = "NativeOCKImplementation.CIPHER_encryptFinal";
    ...
    if ((NULL == ciphertextNative) ||
        ...
        returnResult = CIPHER_encryptFinal_internal(
            ockCtx, ockCipher, plaintextNative + (int)plaintextOffset,
            (int)plaintextLen, ciphertextNative + (int)ciphertextOffset,
            (bool)needsReinit);
        if (CIPHER_INTERNAL_SUCCESS > returnResult) {
...
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1decryptUpdate(
    ...
    static const char *functionName = "NativeOCKImplementation.CIPHER_decryptUpdate";
    ...
    if (NULL == ciphertextNative || NULL == plaintextNative) {
        ...
        returnResult = CIPHER_decryptUpdate_internal(
            ockCtx, ockCipher, ciphertextNative + (int)ciphertextOffset,
            (int)ciphertextLen, plaintextNative + (int)plaintextOffset,
            (bool)needsReinit);
        if (CIPHER_INTERNAL_SUCCESS != returnResult) {
            ockCheckStatus(ockCtx);
        }
...
JNIEXPORT int CIPHER_decryptFinal_internal(
    ...
    rc = ICC_EVP_DecryptFinal(ockCtx, ockCipher->cipherCtx,
...
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1decryptFinal(
    ...
    jbyteArray ciphertext, jint ciphertextOffset, jint ciphertextLen,
    jbyteArray plaintext, jint plaintextOffset, jboolean needsReinit) {
    static const char *functionName = "NativeOCKImplementation.CIPHER_decryptFinal";
    ...
    if ((NULL == plaintextNative) ||
        ...
        returnResult = CIPHER_decryptFinal_internal(
            ockCtx, ockCipher, ciphertextNative + (int)ciphertextOffset,
            (int)ciphertextLen, plaintextNative + (int)plaintextOffset,
            (bool)needsReinit);
...
JNIEXPORT jlong JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_checkHardwareSupport(
    JNIEnv *env, jclass thisObj, jlong ockContextId) {
    int      rv  = 0;
    ICC_CTX *ctx = (ICC_CTX *)((intptr_t)ockContextId);

    static const char *functionName = "NativeOCKImplementation.checkHardwareSupport";
    ...
    if ((NULL == funcPtr[0].func) || (NULL == funcPtr[0].name)) {
        ...
        if (debug) {
            gslogMessage("funcPtr[0].func is '0x%p' funcPtr[0].name is '%s'",
                         funcPtr[0].func, funcPtr[0].name);
        }
    }
#ifdef __MVS__
#pragma convert("ISO8859-1")
...
#pragma convert(pop)
        ...
        if (debug) {
            gslogMessage(
                "Function mismatch, expected presence  did not get it got %s",
                funcPtr[0].name);
            gslogFunctionExit(functionName);
        }
        return -1;
    ...
    if (debug) {
        gslogMessage(
            "calling funcPtr[0] and check for rv presence functions \n");
    }
    ...
    if (42 != rv) {
        if (debug) {
            gslogMessage(
                "presence function did not return expected 42 got %d\n", rv);
            gslogFunctionExit(functionName);
        }
        return -1;
    } else {
 ...
 */
JNIEXPORT void JNICALL
Java_com_ibm_crypto_plus_provider_ock_NativeOCKImplementation_CIPHER_1delete(
    JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId) {
    static const char *functionName = "NativeOCKImplementation.CIPHER_delete";
