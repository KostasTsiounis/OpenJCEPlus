...
ICC_AES_GCM_CTX* getOrfreeGCMContext(ICC_CTX* ockCtx, int keyLen) {
    ...
    if (keyLen > 0) {
        ...
        if (isFips == 0) {
            gcmCtxPointer =
                (keyLen == 16 ? &gcmCtx16
                              : (keyLen == 24 ? &gcmCtx24 : &gcmCtx32));
        } else {
            gcmCtxPointer =
                (keyLen == 16 ? &FIPSgcmCtx16
                              : (keyLen == 24 ? &FIPSgcmCtx24 : &FIPSgcmCtx32));
        }

        if (*gcmCtxPointer == NULL) {
            *gcmCtxPointer = ICC_AES_GCM_CTX_new(ockCtx);
            rc             = ICC_AES_GCM_CTX_ctrl(ockCtx, *gcmCtxPointer,
                                                  ICC_AES_GCM_CTRL_TLS13, 0, NULL);
 ...
 * Signature: (JJ)V
 */
int GCM_InitForUpdateDecrypt_core(JNIEnv* env, ICC_CTX* ockCtx,
                                  ICC_AES_GCM_CTX* gcmCtx, unsigned char* key,
    ...
    if (debug) {
        gslogMessage(
            "DETAIL_GCM NI GCM_InitForUpdateDecrypt_core gcmCtx %ld keyLen %d "
            "ivLen %d aadLen %d updateOutlen %ld\n",
    ...
    if (rc == ICC_OSSL_SUCCESS) {
        ...
        if (rc == ICC_OSSL_SUCCESS) {
            if (aadLen > 0) {
                ...
                if (debug) {
                    gslogMessage(
                        "DETAIL_GCM NI calling ICC_AES_GCM_DecryptUpdate for "
                        "AAD\n");
                }
#endif
                ...
                if (rc != ICC_OSSL_SUCCESS) {
                    ...
                    if (debug) {
                        gslogMessage(
                            "NI data ICC_AES_GCM_DecryptUpdate "
                            "ICC_AES_GCM_CRYPTUPDATE_FAILED\n");
                    }
#endif
                    ...
                    if (debug) {
                        gslogMessage(
                            "DETAIL_GCM NI AAD ICC_AES_GCM_DecryptUpdate "
                            "Succeeded\n");
                    }
#endif
                }
                ...
                if (debug) {
                    gslogMessage(
                        "DETAIL_GCM NI No AAD ICC_AES_GCM_DecryptUpdate not "
                        "called \n");
                }
#endif
            }
            ...
            if (debug) {
                gslogMessage(
                    "DETAIL_GCM NI ICC_AES_GCM_DecryptUpdate "
                    "ICC_AES_GCM_INIT_FAILED \n");
            }
#endif
 ...
 * Signature:
 */
int GCM_UpdForUpdateDecrypt_core(JNIEnv* env, ICC_CTX* ockCtx,
                                 ICC_AES_GCM_CTX* gcmCtx, unsigned char* data,
    ...
    *updateOutlen = 0;
    ...
    if (debug) {
        gslogMessage(
            "DETAIL_GCM NI GCM_UpdForUpdateDecrypt_core gcmCtx %ld  dataOffset "
            "%d dataLen %d outOffset %d updateOutlen %ld\n",
            gcmCtx, dataOffset, dataLen, outOffset, *updateOutlen);
    }
#endif
    if (rc == ICC_OSSL_SUCCESS) {
#ifdef DEBUG_GCM_DETAIL
        ...
        if (dataLen > 0) {
            ...
            if (debug) {
                gslogMessage(
                    "DETAIL_GCM NI ICC_AES_GCM_DecryptUpdate dataLen > 0 "
                    "dataLen = %d outOffset %d updateOutlen %ld\n",
                    dataLen, dataOffset, *updateOutlen);
            }
#endif
            rc = ICC_AES_GCM_DecryptUpdate(ockCtx, gcmCtx, NULL, 0,
            ...
            if (debug) {
                gslogMessage(
                    "DETAIL_GCM NI data update returned "
                    "ICC_AES_GCM_DecryptUpdate returns rc = %d updateOutLen = "
                    "%ld\n",
                    rc, *updateOutlen);
            }
#endif
            ...
            if (rc != ICC_OSSL_SUCCESS) {
                ...
                if (debug) {
                    gslogMessage(
                        "DETAIL_GCM NI data ICC_AES_GCM_DecryptUpdate "
                        "Succeeded\n");
                }
#endif
            }
 ...
 * Signature:
 */
int GCM_InitForUpdateEncrypt_core(JNIEnv* env, ICC_CTX* ockCtx,
                                  ICC_AES_GCM_CTX* gcmCtx, unsigned char* key,
                                  int keyLen, unsigned char* iv, int ivLen,
                                  unsigned char* aad, int aadLen,
                                  unsigned long* updateOutlen) {
    ...
    if (debug) {
        gslogMessage(
            "DETAIL_GCM NI GCM_InitForUpdateEncrypt_core updateOutlen %ld\n",
            *updateOutlen);
    ...
    if (debug) {
        ...
        gslogMessage("DETAIL_GCM rc ICC_AES_GCM_CTX_new %d gcmCtx=%x", (int)rc,
                     gcmCtx);
    }
#endif
    if (rc == ICC_OSSL_SUCCESS) {
        // Do initialization
        rc = ICC_AES_GCM_Init(ockCtx, gcmCtx, ivNative, ivLen, keyNative,
                              keyLen);
#ifdef DEBUG_GCM_DETAIL
        if (debug) {
        ...
        if (rc == ICC_OSSL_SUCCESS) {
            if (aadLen > 0) {
                ...
                if (debug) {
                    gslogMessage(
                        "DETAIL_GCM rc  ICC_AES_GCM_EncryptUpdateCalled with "
                        "AADLen %d\n",
                        aadLen);
                }
#endif
                ...
                rc =
                    ICC_AES_GCM_EncryptUpdate(ockCtx, gcmCtx, aadNative, aadLen,
                                              NULL, 0, NULL, &updateAADlen);
#ifdef DEBUG_GCM_DETAIL
                if (debug) {
                    gslogMessage(
                        "DETAIL_GCM rc ICC_AES_GCM_EncryptUpdate(aadLen > 0) "
                        "%d updateAADlen %d",
                        (int)rc, updateAADlen);
                }
#endif
                if (rc != ICC_OSSL_SUCCESS) {
                    ...
                    if (debug) {
                        gslogMessage(
                            "DETAIL_GCM ICC_AES_GCM_EncryptUpdate call for AAD "
                            "succeded\n");
                    }
#endif
                }
                ...
                if (debug) {
                    gslogMessage(
                        "DETAIL_GCM rc  ICC_AES_GCM_EncryptUpdate not called "
                        "for AAD\n");
                }
#endif
            }
 ...
 * Signature:
 */
int GCM_UpdForUpdateEncrypt_core(JNIEnv* env, ICC_CTX* ockCtx,
                                 ICC_AES_GCM_CTX* gcmCtx, unsigned char* data,
    ...
    if (rc == ICC_OSSL_SUCCESS) {
        if (dataLen > 0) {
            ...
            if (debug) {
                gslogMessage(
                    "NI ICC_AES_GCM_EncryptUpdateCalled with data %d\n",
                    dataLen);
            }
#endif
            // update data
            rc = ICC_AES_GCM_EncryptUpdate(ockCtx, gcmCtx, NULL, 0,
                                           data + dataOffset, dataLen,
                                           out + outOffset, updateOutlen);
#ifdef DEBUG_GCM_DETAIL
            if (debug) {
                gslogMessage(
                    "NI GCM_UpdForUpdateEncrypt_core dataLen > 0 path after "
                    "ICC_AES_GCM_EncryptUpdate rc %d updateOutlen %ld\n",
                    rc, *updateOutlen);
                gslogMessage(
                    "DETAIL_GCM rc ICC_AES_GCM_EncryptUpdate(plaintextLen > 0) "
                    "%d updateOutlen %d",
                    (int)rc, *updateOutlen);
            }
#endif
 ...
 * Signature:
 */
int GCM_FinalForUpdateDecrypt_core(JNIEnv* env, ICC_CTX* ockCtx,
                                   ICC_AES_GCM_CTX* gcmCtx, unsigned char* data,
                                   int dataOffset, int dataLen,
                                   unsigned char* out, int outOffset,
    ...
    if (debug) {
        gslogMessage(
            "NI NativeInterface GCM_FinalForUpdateDecrypt_core gcmCtx = %ld "
            "dataOffset %d dataLen %d outOffset %d tagLen %d updateOutlen "
            "%ld\n",
    ...
    if (rc == ICC_OSSL_SUCCESS) {
        if (dataLen > 0) {
            ...
            if (debug) {
                gslogMessage(
                    "NI calling ICC_AES_GCM_DecryptUpdate  dataOffset %d "
                    "dataLen %d outputOffset %d updateOutlen %ld\n",
                    dataOffset, dataLen, outOffset, updateOutlen);
            }
#endif
            if ((dataLen - tagLen) > 0) {
                rc = ICC_AES_GCM_DecryptUpdate(
                    ockCtx, gcmCtx, NULL, 0, data + dataOffset,
                    dataLen - tagLen, out + outOffset, &updateOutlen);
            }
            ...
            if (rc == ICC_OSSL_SUCCESS) {
                 ...
                 * compare */
                // rc = ICC_AES_GCM_DecryptFinal(ockCtx, gcmCtx, out + outOffset
                // + updateOutlen, &finalOutlen,data + dataOffset + dataLen,
                // tagLen);
#ifdef DEBUG_GCM_DETAIL
                if (debug) {
                    gslogMessage(
                        "NI calling ICC_AES_GCM_DecryptFinal updateOutlen "
                        "%ld\n",
                        updateOutlen);
                }
#endif
                rc = ICC_AES_GCM_DecryptFinal(
                    ockCtx, gcmCtx, out + outOffset + updateOutlen,
                    &finalOutlen, data + dataOffset + dataLen - tagLen, tagLen);
#ifdef DEBUG_GCM_DETAIL
                if (debug) {
                    gslogMessage(
                        "NI ICC_AES_GCM_DecryptFinal returns rc %d finalOutlen "
                        "%d\n",
                        rc, finalOutlen);
                }
#endif
                if (rc != ICC_OSSL_SUCCESS) {
                    ...
                    if (debug) {
                        gslogMessage(
                            "NI ICC_AES_GCM_DecryptFinal has encountered error "
                            "condition\n");
                    }
#endif
                    if (rc == -1) {
                        ...
                        if (debug) {
                            gslogMessage(
                                "NI ICC_AES_GCM_DecryptFinal returning "
                                "ICC_AES_GCM_TAG_MISMATCH\n");
                        }
#endif
                    ...
                    if (debug) {
                        gslogMessage(
                            "NI ICC_AES_GCM_DecryptFInal was successful\n");
                    }
#endif
                }
            } else {
            ...
            if (debug) {
                gslogMessage(
                    "NI ICC_AES_GCM_DecryptUpdate returning "
                    "ICC_AES_GCM_CTX_NEW_FAILED\n");
            }
#endif
            return ICC_AES_GCM_CTX_NEW_FAILED;
        }
 ...
 * Signature:
 */
int GCM_FinalForUpdateEncrypt_core(JNIEnv* env, ICC_CTX* ockCtx,
                                   ICC_AES_GCM_CTX* gcmCtx, unsigned char* tag,
                                   int tagLen, unsigned char* dataText,
                                   int dataOffset, int dataLen,
                                   unsigned char* out, int outOffset,
                                   unsigned long updateOutlen) {
#if defined(AIX) || defined(WINDOWS) || defined(MAC) || defined(LINUX) || \
    ...
    if (rc == ICC_OSSL_SUCCESS) {
        if (dataLen > 0) {
            ...
            if (debug) {
                gslogMessage(
                    "DETAIL_GCM NI calling data Len > 0 calling "
                    "ICC_AES_GCM_EncryptUpdate before ICC_AES_GCM_EncryptFinal "
                    " dataOffset %d dataLen %d outputOffset %d updateOutlen "
                    "%ld\n",
                    dataOffset, dataLen, outOffset, updateOutlen);
            }
#endif
            rc = ICC_AES_GCM_EncryptUpdate(
                ockCtx, gcmCtx, NULL, 0, dataNative + dataOffset, dataLen,
                outNative + outOffset, &updateOutlen);

#ifdef DEBUG_GCM_DETAIL
            if (debug) {
                gslogMessage(
                    "DETAIL_GCM NI rc ICC_AES_GCM_EncryptUpdate returned %d "
                    "updateOutlen %ld\n",
                    rc, updateOutlen);
            }
#endif
        ...
        if (rc == ICC_OSSL_SUCCESS) {
            ...
            if (debug) {
                gslogMessage(
                    "DETAIL_GCM NI GCM_FinalForUpdateEncrypt_core updateOutlen "
                    "%ld\n",
                    updateOutlen);
            }
#endif
            rc = ICC_AES_GCM_EncryptFinal(ockCtx, gcmCtx,
                                          outNative + outOffset + updateOutlen,
                                          &finalOutlen, tagNative);
#ifdef DEBUG_GCM_DETAIL
            if (debug) {
                gslogMessage(
                    "DETAIL_GCM NI return rc %d from ICC_AES_GCM_EncryptFinal "
                    "finalOutlen %ld updateOutlen %ld outOffset %d\n",
...
void z_km_native(signed char* in, int inputLength, int inputOffset,
                 signed char* out, int outputOffset, signed char* parm_block,
                 long mode) {
...
void z_kimd_native(signed char* in, int inputLength, int inputOffset,
...
void handleIV(int ivLength, int keyLen, int blockSize, int J0Offset, char* iv,
              char* key, char* addedParams) {
    ...
    if (ivLength == 12) {
        ...
        z_km_native((signed char*)&zeros, blockSize, 0,
        ...
        if (ivLength >= blockSize) {
            ...
            z_kimd_native((signed char*)iv, lenn, offset,
...
int checkTagMismatch(char* input, int inputLen, char* parm_block, int tagOffset,
                     int tagLen) {
#if defined(S390_PLATFORM) || defined(__MVS__)
...
int GCM_decrypt_core(JNIEnv* env, ICC_CTX* ockCtx, ICC_AES_GCM_CTX* gcmCtx,
                     ...
                     int ivLen, unsigned char* ciphertext, int ciphertextOffset,
                     int ciphertextLen, unsigned char* plaintext,
                     int plaintextOffset, unsigned char* aad, int aadLen,
                     int tagLen) {
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_do_1GCM_1decryptFastJNI_1WithHardwareSupport(
    ...
    handleIV(ivLen, keyLen, blockSize, J0Offset, (char*)iv,
    ...
    zS390((input != NULL) ? (input + inputOffset) : NULL,
          (output != NULL) ? (output + outputOffset) : NULL, aad, &len, &alen,
          parm_block, &mode);

    ret =
        checkTagMismatch((input != NULL) ? (char*)(input + inputOffset) : NULL,
                         len, (char*)parm_block, tagOffset, tagLen);
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_do_1GCM_1decryptFastJNI(
    ...
    jlong inputBuffer, jlong outputBuffer) {
    ...
    ret = GCM_decrypt_core(env, ockCtx, gcmCtx, key, keyLen, iv, ivLen,
                           ciphertext, ciphertextOffset, ciphertextLen,
                           plaintext, plaintextOffset, aad, aadLen, tagLen);
...
#endif
    if (rc == ICC_OSSL_SUCCESS) {
        ret = GCM_decrypt_core(env, ockCtx, gcmCtx, keyNative, keyLen, ivNative,
                               ivLen, ciphertextNative, ciphertextOffset,
                               ciphertextLen, plaintextNative, plaintextOffset,
...
int GCM_encrypt_core(JNIEnv* env, ICC_CTX* ockCtx, ICC_AES_GCM_CTX* gcmCtx,
                     unsigned char* key, int keyLen, unsigned char* iv,
                     int ivLen, unsigned char* aad, int aadLen,
                     unsigned char* tag, int tagLen, unsigned char* plain,
                     int plaintextLen, int plaintextOffset,
    ...
    if (rc == ICC_OSSL_SUCCESS) {
        ...
        if (rc == ICC_OSSL_SUCCESS) {
            if ((aadLen > 0) && (plaintextLen > 0)) {
                rc = ICC_AES_GCM_EncryptUpdate(
                    ockCtx, gcmCtx, aadNative, aadLen,
                ...
                if (debug) {
                    gslogMessage(
                        "DETAIL_GCM rc ICC_AES_GCM_EncryptUpdate(aadLen > 0 & "
                        "plaintextLen > 0) %d updateOutlen %d",
                        (int)rc, updateOutlen);
                }
#endif
            } else {
                if (aadLen > 0) {
                    // update AAD
                    rc = ICC_AES_GCM_EncryptUpdate(ockCtx, gcmCtx, aadNative,
                                                   aadLen, NULL, 0, NULL,
                                                   &updateAADlen);
#ifdef DEBUG_GCM_DETAIL
                    if (debug) {
                        gslogMessage(
                            "DETAIL_GCM rc ICC_AES_GCM_EncryptUpdate(aadLen > "
                            "0) %d updateAADlen %d",
                            (int)rc, updateAADlen);
                    }
#endif
                ...
                if (plaintextLen > 0) {
                    ...
                    if (debug) {
                        gslogMessage(
                            "DETAIL_GCM rc "
                            "ICC_AES_GCM_EncryptUpdate(plaintextLen > 0) %d "
                            "updateOutlen %d",
                            (int)rc, updateOutlen);
                    }
#endif
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_do_1GCM_1encryptFastJNI(
    ...
    ret = GCM_encrypt_core(env, ockCtx, gcmCtx, key, keyLen, iv, ivLen, aad,
                           aadLen, tag, tagLen, plaintextNative, plaintextLen,
                           plaintextOffset, ciphertextNative, ciphertextOffset);
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_do_1GCM_1encrypt(
    ...
    rc = (ivNative == NULL || keyNative == NULL || ciphertextNative == NULL ||
          tagNative == NULL || aadNative == NULL)
             ? ICC_OSSL_FAILURE
             : ICC_OSSL_SUCCESS;

    if (rc == ICC_OSSL_SUCCESS) {
        ret = GCM_encrypt_core(env, ockCtx, gcmCtx, keyNative, keyLen, ivNative,
                               ivLen, aadNative, aadLen, tagNative, tagLen,
...
/* Return to Java Space a flag informing if TLS (Thread Local Storage) is used
 * by the native GCM code or not.
 *
 * Currently this is controlled per platform since there is a compiler bug in
 * AIX that prevents TLS use. TLS use in native code should be preferred as it
...
#endif
        ret = GCM_UpdForUpdateEncrypt_core(
            env, ockCtx, gcmCtx, plaintextNative, plaintextLen, plaintextOffset,
            ciphertextNative, ciphertextOffset, &updateOutlen);
#ifdef DEBUG_GCM_DETAIL
        if (debug) {
            gslogMessage(
                "NI ret from GCM_update_encrypt_core  %d modified updateOutlen "
                "%ld ICC_OSSL_SUCCESS=%d\n",
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_do_1GCM_1FinalForUpdateEncrypt(
    ...
    ciphertextNative = (unsigned char*)((*env)->GetPrimitiveArrayCritical(
        env, ciphertext, &isCopy));
    if (aadLen > 0) {
        aadNative = (unsigned char*)((*env)->GetPrimitiveArrayCritical(
            env, aad, &isCopy));
    }
    tagNative =
        (unsigned char*)((*env)->GetPrimitiveArrayCritical(env, tag, &isCopy));
    ...
    rc = (ivNative == NULL || keyNative == NULL || ciphertextNative == NULL ||
          tagNative == NULL || ((aadLen > 0) && (aadNative == NULL)) ||
          ((plaintextLen > 0) && (plaintextNative == NULL)))
             ? ICC_OSSL_FAILURE
             : ICC_OSSL_SUCCESS;
...
#endif
        ret = GCM_FinalForUpdateEncrypt_core(
            env, ockCtx, gcmCtx, tagNative, tagLen, plaintextNative,
            plaintextOffset, plaintextLen, ciphertextNative, ciphertextOffset,
            updateOutlen);
#ifdef DEBUG_GCM_DETAIL
        if (debug) {
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_do_1GCM_1UpdForUpdateDecrypt(
    ...
    rc = (((ciphertextLen > 0) && (ciphertextNative == NULL)) ||
          plaintextNative == NULL)
             ? ICC_OSSL_FAILURE
             : ICC_OSSL_SUCCESS;
#ifdef DEBUG_GCM_DETAIL
    if (debug) {
    ...
    if (rc == ICC_OSSL_SUCCESS) {
        ...
        if (debug) {
            gslogMessage(
                "DETAIL_GCM NI calling GCM_UpdForUpdateDecrypt_core "
                "ciphertextOffSet %d ciphertextLen %d plaintextOffSet %d \n",
                ciphertextOffset, ciphertextLen, plaintextOffset);
        }
#endif
        ret = GCM_UpdForUpdateDecrypt_core(
            env, ockCtx, gcmCtx, ciphertextNative, ciphertextOffset,
            ciphertextLen, plaintextNative, plaintextOffset, &updateOutlen);
#ifdef DEBUG_GCM_DETAIL
        if (debug) {
            gslogMessage(
                "DETAIL_GCM NI GCM_update_decrypt_core returns %d updateOutlen "
                "%ld\n",
                ret, updateOutlen);
        }
#endif
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_do_1GCM_1InitForUpdateDecrypt(
    ...
    if (debug) {
        gslogMessage(
            "DETAIL_GCM NI do_GCM_InitForUpdateDecrypt ockContextId %ld "
            "gcmCtxId %ld keyLen %d ivLen %d aadLen %d\n",
    ...
    rc = (ivNative == NULL || keyNative == NULL ||
          ((aadLen > 0) && (aadNative == NULL)))
             ? ICC_OSSL_FAILURE
             : ICC_OSSL_SUCCESS;
#ifdef DEBUG_GCM_DETAIL
    if (debug) {
    ...
    if (rc == ICC_OSSL_SUCCESS) {
        ...
        if (debug) {
            gslogMessage(
                "DETAIL_GCM NI calling GCM_InitForUdateDecryptCore keyLen %d "
                "ivLen %d addLen %d \n",
                keyLen, ivLen, aadLen);
        }
#endif
        ret = GCM_InitForUpdateDecrypt_core(env, ockCtx, gcmCtx, keyNative,
                                            keyLen, ivNative, ivLen, aadNative,
                                            aadLen, &updateOutlen);
#ifdef DEBUG_GCM_DETAIL
        if (debug) {
            gslogMessage(
                "DETAIL_GCM NI GCM_InitForUpdateDecrypt_core returns %d "
                "updateOutlen %ld\n",
                ret, updateOutlen);
        }
#endif
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_do_1GCM_1FinalForUpdateDecrypt(
    ...
    if (debug) {
        gslogMessage(
            "NI DETAIL_GCM   do_GCM_FinalForUpdateDecrypt updateOutlen %ld "
            "ciphertexLen %d, plaintextLen %d aadLen %d\n",
...
#endif
        ret = GCM_FinalForUpdateDecrypt_core(
            env, ockCtx, gcmCtx, ciphertextNative, ciphertextOffset,
            ciphertextLen, plaintextNative, plaintextOffset, tagLen,
            updateOutlen);
#ifdef DEBUG_GCM_DETAIL
        if (debug) {
            gslogMessage(
                "DETAIL_GCM GCM_FinalForUdateDerypt_core ret = %d updateOutLen "
                "= %ld\n",
                rc, updateOutlen);
        }
#endif
...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_do_1GCM_1InitForUpdateEncrypt(
    ...
    if (rc == ICC_OSSL_SUCCESS) {
        ...
        if (debug) {
            gslogMessage(
                "DETAIL_GCM NI calling GCM_InitForUdateEncrypt_core keyLen %d "
                "ivLen %d  addLen %d \n",
                keyLen, ivLen, aadLen);
        }
#endif
        ret = GCM_InitForUpdateEncrypt_core(env, ockCtx, gcmCtx, keyNative,
                                            keyLen, ivNative, ivLen, aadNative,
                                            aadLen, &updateOutlen);
#ifdef DEBUG_GCM_DETAIL
        if (debug) {
            gslogMessage(
                "DETAIL_GCM NI GCM_InitForUpdateEncrypt_core returns %d "
                "updateOutlen %ld\n",
                ret, updateOutlen);
        }
#endif
